...
class ProfileViewController: UIViewController {
    ...
    var whiteTextColor = UIColor(displayP3Red: 187/255, green: 225/255, blue: 250/255, alpha: 1)
        ...
        func initUI(){
            ...
            backButton = UIButton(frame: CGRect(x: 10, y: 85, width: 20, height: 20))
            backButton?.setImage(UIImage(named: "backButton"), for: .normal)
            backButton?.addTarget(self, action: #selector(backAction), for: .touchUpInside)
            view.addSubview(backButton!)
            
            backText = UILabel(frame: CGRect(x: 37, y: 80, width: 100, height: 50))
        ...
        func createProductView(){
            ...
            tituloLabel?.text = "Marco A. Cruz "
            tituloLabel?.textColor = .skyBlue
